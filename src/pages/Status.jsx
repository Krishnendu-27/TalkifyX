...
const MOCK_STATUSES = [
    ...
    stories: [
      {
        id: "s1",
        type: "image",
        url: "https://images.unsplash.com/photo-1516641396056-0ce60a85d49f?w=600&h=900&fit=crop",
        duration: 5000,
      },
      {
        id: "s2",
    ...
    stories: [
      {
        id: "s3",
        type: "image",
        url: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=600&h=900&fit=crop",
        duration: 5000,
      },
    ],
  },
  {
    ...
    createdAt: new Date(new Date().setDate(new Date().getDate() - 1)).setHours(
      23,
      45,
      0,
      0
    ...
    stories: [
      {
        id: "s4",
        type: "image",
        url: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=600&h=900&fit=crop",
        duration: 5000,
      },
    ],
  },
...
const Status = () => {
  ...
  const handleSelectStatus = (status) => {
    ...
    const firstUnviewedIndex = status.stories.findIndex(
  ...
  const handleNextStory = (e) => {
    ...
    if (
      selectedStatus &&
      viewingStoryIndex < selectedStatus.stories.length - 1
    ) {
  ...
  return (
    <div className={`flex h-screen w-full overflow-hidden ${theme.navBg}`}>
      ...
      >
        {/* Header */}
        <div
          className={`flex items-center justify-between px-4 py-4 ${theme.navBg} border-b ${theme.divider} shadow-sm`}
        >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-0 pb-20 md:pb-0">
          ...
          {/* Section Divider */}
          <div
            className={`px-4 py-2 text-sm font-medium ${theme.textMuted} uppercase tracking-wider text-[11px] mt-2`}
          >
            Recent updates
          </div>

          {/* Recent Updates List */}
          {MOCK_STATUSES.filter((s) =>
          ...
          {/* Section Divider */}
          <div
            className={`px-4 py-2 text-sm font-medium ${theme.textMuted} uppercase tracking-wider text-[11px] mt-4`}
          >
            Viewed updates
          </div>
          ...
          {MOCK_STATUSES.filter((s) =>
            ...
            .map((status) => (
              ...
              >
                <div
                  className={`w-12 h-12 flex-shrink-0 flex items-center justify-center p-[3px] rounded-full border-2 ${theme.divider} relative`}
                >
                  <div className="w-full h-full rounded-full overflow-hidden">
                    <img
                      src={status.user.avatar}
                ...
                <div className="flex-1">
                  <h3
                    className={`font-medium ${theme.text} text-base opacity-90`}
                  >
                    {status.user.username}
                  </h3>
                  <p className={`text-sm ${theme.textMuted}`}>
      ...
      <div
        className={`${
          selectedStatus
            ? "fixed inset-0 z-[60] flex bg-black md:static md:z-auto"
            : "hidden md:flex"
      ...
      >
        {selectedStatus ? (
          <div className="relative w-full h-full flex items-center justify-center bg-zinc-900">
            ...
            >
              {selectedStatus.stories[viewingStoryIndex].type === "image" ? (
                ...
                <div
                  className={`w-full h-full flex items-center justify-center text-white text-2xl font-bold p-10 text-center ${selectedStatus.stories[viewingStoryIndex].bg}`}
                >
          ...
          <div className="flex flex-col items-center justify-center text-center opacity-70 select-none">
            ...
            <h2 className="text-xl text-white/80 font-medium mb-2">
